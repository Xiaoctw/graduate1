...
_, train_x, _, _, test_x, _, train_y, test_y = pre_data('Chicago')
...
gbm,model=construct_GBDT_Dense(train_x=train_x,train_y=train_y,test_x=test_x,test_y=test_y,
                               field_size=field_size,feat_sizes=feat_sizes,num_epoch=40,
...
for i in range(num_update):
    ...
    model.update_model(train_x,train_y,batch_x,batch_y)
len1=len(roc_es)
pred_test=gbm.predict(data=test_x)
gbdt_roc=roc_auc_score(test_y,pred_test)
gbdt=[gbdt_roc]*len1
x_labels=[i+1 for i in range(len1)]
plt.plot(x_labels, roc_es, color='g', label='deep_model', lw=2, ls='-')
